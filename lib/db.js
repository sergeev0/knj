  ...
  return boards.map((board) => {
    const columns = connection
      ...
      .map((column) => ({
        ...
        profile: column.profile_id
          ? {
              id: column.profile_id,
              harness: column.harness,
              model: column.model,
              prompt: column.prompt,
              allowedTools: column.allowed_tools,
              contextRules: column.context_rules,
              workingDirectory: column.working_directory,
              environment: column.environment,
              completionCriteria: column.completion_criteria
            }
          : null
      }));
...
export function updateAgentProfile({
  ...
  connection.prepare(
    `UPDATE agent_profiles
     SET model = ?, prompt = ?, allowed_tools = ?, context_rules = ?,
  ...
  insertAudit(connection, {
    ...
    payload: { processingColumnId, profileId, model }
  });
}
