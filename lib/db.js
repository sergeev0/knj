...
const agentConfigGuidance = [
  "<!--",
  "KNJ agent column config.",
  "Keep the metadata keys and section headings unchanged; KNJ parses them by name.",
  "Edit values inside the sections, not headings such as '## Instructions'.",
  "-->"
].join("\n");
...
function withAgentConfigGuidance(markdown) {
  if (markdown.includes("KNJ agent column config.")) {
    return markdown;
  }

  return `${agentConfigGuidance}\n\n${markdown}`;
}
...
function serializeAgentConfigMarkdown({
  ...
  return withAgentConfigGuidance([
    "# KNJ Agent Column Config",
    "",
...
function loadAgentProfile(connection, processingColumnId) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    const guidedMarkdown = withAgentConfigGuidance(markdown);
    if (guidedMarkdown !== markdown) {
      fs.writeFileSync(filePath, guidedMarkdown);
    }

    const fileProfile = parseAgentConfigMarkdown(guidedMarkdown);
